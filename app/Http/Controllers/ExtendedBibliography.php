...
{
    public static function create($r, $item)
    {
        // $p = [$r[$item->item_type]];
        $p = $r[$item->item_type];
    ...
    public static function update($r, $item)
    {
        // $p = [$r[$item->item_type]];
        $p = $r[$item->item_type];
        ...
        if ($item->item_type === 'audio') {
            ...
            $i = Book::where('item_id', $item->id)->first();

            $i->isbn_issn = $p['isbn_issn'] ?? $i->isbn_issn;
            $i->edition = $p['edition'] ?? $i->edition;
            $i->pages = $p['pages'] ?? $i->pages;
            $i->save();
        } elseif ($item->item_type === 'thesis') {
            Thesis::create([
                'item_id' => $item->id,
